...
class Data():
    def __init__(self, args, device):
        ...
        if args.dataset_type == 'blender':
            ...
            images, poses, bds, render_poses, i_test = load_llff_data(self.datadir, args.factor,
                                                                  recenter=True, bd_factor=.75,
                                                                  spherify=args.spherify, half_res=args.half_res)
            masks = torch.ones(images.shape[0], images.shape[1], images.shape[2], images.shape[3])
            hwf = poses[0, :3, -1]
            poses = poses[:, :3, :4]
    ...
    def genpc(self):
        [H, W, focal] = self.hwf
        K = torch.tensor(self.K).to(self.device)
        train_n = self.i_split[0].shape[0]
        poses = torch.tensor(self.poses).to(self.device)[:train_n]
        images = torch.tensor(self.images)[:train_n]

        pc,color,N = [],[],np.min([self.images.shape[1], self.images.shape[2]])
        [xs,ys,zs],[xe,ye,ze] = [-2,-2,-2],[2,2,2]
        pts_all = []
        for h_id in tqdm(range(N)):
            i, j = torch.meshgrid(torch.linspace(xs, xe, N).to(self.device),
                                  torch.linspace(ys, ye, N).to(self.device))  # pytorch's meshgrid has indexing='ij'
            i, j = i.t(), j.t()
            pts = torch.stack([i, j, torch.ones_like(i).to(self.device)], -1)
